...
function musicCtrl($scope, $http) {
  ...
  $scope.rdio_tag = function(token, songname){
    ...
    $scope.tag_object.song_data = $scope.results
    $http.post('/addrdiotag', $scope.tag_object.song_data).success(function(data){
      console.log(data)
  ...
  $scope.play_rdio = function(token){
    ...
    console.log(token)
    if(token){
      $scope.play_token.splice(0, 1, token)
    }
